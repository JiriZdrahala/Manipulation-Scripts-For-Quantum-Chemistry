...
def plot_ecd_spectra(exp_file, calc_file,  calc_file_cluster, isRoa, output_image):
    ...
    ssimil_ypos=0.78
    ax2.plot(
        x_calc, y_calc, color="blue", linestyle="-", linewidth=1.5, label="Purely electronic"
    )
    ...
    ax3.plot(
        x_calc_cluster, y_calc_cluster, color="red", linestyle="-", linewidth=1.5, label="Vibronic 3rd state"
    )
...
if __name__ == "__main__":
    ...
    if(isROA[0].lower()=='t'):
        ...
        isROA=False
    print("ROA: "+str(isROA))
    plot_ecd_spectra(EXPERIMENTAL_FILE, PurelyElectronic,  Vibronic,  isROA, OUTPUT_FILE)
